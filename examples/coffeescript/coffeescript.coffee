...
# @route "posts" - You can specify a controller, but in this case
#		the controller has the same name as the template. So "posts"
#		will by default call PostsController. <- Notice the capital P.
# @route "showPost" - No need to create a named controller. You can
# 		just omit the name of the controller and this will create an 
#  	anonymous controller, which allows you to pass in you parameters.
# 		Note: by using the helper pathFor 'showPost' in the posts template, Iron Router
# 		is smart enough to make a url with the correct _id. No need to pass
#     it in.
# @route "dashboard" - A RouteController subclass and a custom action
# 		are passed in. The custom action will render the dashHeader template
