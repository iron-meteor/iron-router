...
Route.prototype = {
  ...
  resolve: function (params, options) {
        ...
        .replace(
          ...
          function (match, slash, format, key, capture, optional, offset) {
            ...
            if (optional && !isValueDefined) {
              // If we have /:paramOne/:paramTwo? and paramOne == a, we want /a and not /a/
              // But if we have /:paramOne? and paramOne is not defined, we want / and not an empty string
              if (offset !== 0) slash = '';
              value = '';
    ...
    // optional paths can resolve down to nothing due to stripping slashes
    if (path === '')
      path = '/'
