...
Route = function (path, fn, options) {
  var route = function (req, res, next) {
    var controller = this;
    route.dispatch(controller, req.url, {
      req: req, 
      res: res,
      next: next,
      thisArg: controller
  ...
  return ctor.call(this, route, path, fn, options);
};

var ctor = function (route, path, fn, options) {
  if (typeof fn === 'object') {
    options = fn;
    fn = options.action;
  }
  ...
  // the original path
  route._path = Url.normalize(path);

  // track which methods this route uses
  route._methods = {};

  if (fn) {
    route._actionStack.push(route._path, fn, options);
  }
...
Route.prototype.findControllerConstructor = function () {
  ...
  var resolve = function (name, opts) {
    ...
    if (typeof C === 'undefined' && opts.supressErrors !== true)
  ...
  if (typeof this.options.controller === 'object')
 ...
 */
Route.prototype.dispatch = function (controller, url, options) {
  // call runRoute on the controller which will behave similarly to the previous
  // version of IR.
  controller._runRoute(this, url, options);
};
