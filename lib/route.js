...
Route.prototype = {
  ...
  getController: function (path, options) {
    ...
    // XXX: should we be treating after / load / unload like we are treating before?
    // add hooks that have been added by Router.before(...)
    // XXX CM - I think so. I think all hooks should have this interface. It
    // might be confusing for people though. Maybe to start we can do it for
    // before/after and see how it goes.

    //XXX CM - It's probably okay for now, but I think the process of collecting
    //hooks should happen at run time vs. ctor time. There's a method in
    //RouteController called collectHooks and that's probably where we want to
    //put this.
    var routerBeforeHooks = toArray(this.router.options.before);
    _.each(this.router._beforeHooks, function(hook) {
      var options = hook.options;
      
      if (options.except && _.include(options.except, self.name))
        return;
      
      if (options.only && ! _.include(options.only, self.name))
        return;
      
      routerBeforeHooks.push(hook.hook);
    });


    options = _.extend({}, this.router.options, this.options, options || {}, {
      before: routerBeforeHooks
        .concat(toArray(this.options.before)),
      after: toArray(this.router.options.after)
        .concat(toArray(this.options.after)),
      unload: toArray(this.router.options.unload)
        .concat(toArray(this.options.unload)),
