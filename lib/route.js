...
Route.prototype = {
  ...
  findController: function (path, options) {
    ...
    if (_.isObject(this.controller))
      controller = this.controller;

    // no controller specified but we have a name so let's
    // try to find it intelligently in the global namespace
    if (this.name)
      controller = resolveValue(Router.convertRouteControllerName(this.name + 'Controller'));

    // return the controller.
    // Or, no controller? okay just use the base class.
    return controller ? controller : RouteController;
  },
