...
Route.prototype = {
  ...
  getController: function (path, options) {
    ...
    //XXX clean this up it's too hard to manage. maybe we do the hook lookup at
    //route controller _run time.
    options = _.extend({}, this.router.options, this.options, options || {}, {
      onBeforeAction: toArray(this.options.before || this.options.onBeforeAction),
      onAfterAction: toArray(this.options.after || this.options.onAfterAction),
      onStop: toArray(this.options.unload || this.options.onStop),
      onRun: toArray(this.options.load || this.options.onRun),
