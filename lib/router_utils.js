...
//@export RouterUtils.resolveValue
...
RouterUtils.create = function (definition, options) {
  ...
  if (extendFrom) {
    if (options.delayInheritance === false) {
      inherit(Constructor, extendFrom);
    } else {
      savedPrototype = Constructor.prototype;
      // delayed inheritance. this adds this method to a queue and then executes
      // it later, after all the initial scripts have loaded and executed.
      RouteController.onCreateFlush(function () {
        inherit(Constructor, extendFrom, savedPrototype);
      });
    }
  }
