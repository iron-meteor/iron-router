 ...
 * higher layer.
 */
RouteController.prototype.dispatch = function (url, stack, options) {
  var self = this;
  var result;
  options = _.extend({}, this.options, options || {}, {thisArg: this});

  try {
    this._isInDispatch = true;

    Fiber(function () {
      stack.dispatch(url, options);
    }).run();
  } finally {
    this._isInDispatch = false;
  }
};
 ...
 */
RouteController.prototype._runRoute = function (route, url, options) {
  var self = this;
