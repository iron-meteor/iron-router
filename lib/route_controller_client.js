...
RouteController.prototype._runRoute = function (route, url, done) {
  ...
  //XXX make onRun and all hooks behave the same way as middleware stack so
  //you can plug in connect middleware at the right points.
  ...
  stack.append(
    function onRunHooks () {
      if (this._computation.firstRun && !RouteController._hasJustReloaded) {
        var numHooks = this.runHooks('onRun', 'load');
        if (numHooks == 0) this.next();
      } else {
        this.next();
      }
    },

    function onRerunHooks () {
      if (!this._computation.firstRun) {
        var numHooks = this.runHooks('onRerun');
        if (numHooks == 0) this.next();
      } else {
        this.next();
      }
