...
RouteController.prototype._runRoute = function (route, url, done) {
  ...
  var subList = _.map(this._collectHooks('subscriptions'), function (sub) {
    return function () { return sub.call(self); };
  });
  ...
  var waitOnList = _.map(this._collectHooks('waitOn'), function (waitOn) {
    return function () { return waitOn.call(self); };
  });
  ...
  if (waitOnList.length > 0) {
    self.wait(waitOnList);
    useLoadingHook = true;
  }
