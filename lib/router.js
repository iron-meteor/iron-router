...
Router = function (options) {
  ...
  function router (req, res, next) {
    //XXX this assumes no other routers on the
    //parent stack which we should probably fix
 ...
 *
 * @param {String} url The url for the dispatch so we can find the first route
 * for the given url.
 * @param {Object} [options] The options to pass to findFirstroute or the
 * RouteController constructor.
 */
Router.prototype.createController = function (url, options) {
  // see if there's a route for this url
  var route = this.findFirstRoute(url, options);
  var controller;

  options = options || {};

  // so the controller works on our layout instead of creating a new one.
  options.layout = this._layout;

  if (route)
    // let the route decide what controller to use
    controller = route.createController(options);
  else
    // create an anonymous controller
    controller = new RouteController(options);
