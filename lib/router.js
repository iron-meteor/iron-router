IronRouter = function (options) {
  ...
  // an array of global before hooks
  // XXX I think we should put all hooks in one place. Maybe it's called
  // _globalHooks.
  // _globalHooks = {
  //  before: [],
  //  after: [],
  ...
  //  unload: []
  // }
  // And there's probably no reason to keep a separate structure for hooks
  // passed as options. Let's just go to one place to find them.
  this._beforeHooks = [];
};
...
IronRouter.prototype = {
  ...
  configure: function (options) {
    ...
    //XXX CM: pull out the hooks here so we can process them the same way as
    //when calling Router.before, etc.
  ...
  /**
   *
   * Add before hooks to all routes. The hooks will apply to all routes,
  ...
  //XXX CM: Shouldn't the hook function be the first parameter? General paradigm
  //is that options are always the last param. The required param comes first.
  ...
  before: function(options, hook) {
    ...
    this._beforeHooks.push({options: options, hook: hook});
  },
