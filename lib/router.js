...
Router.prototype.createController = function (url, context) {
  ...
  controller.url = context.url || url;
  controller.originalUrl = context.originalUrl || url;
  controller.method = context.request && context.request.method;
...
Router.prototype.addHook = function(type, hook, options) {
  ...
  // Add hook options to "normal" option under supplied
  // namespace or hook name, otherwise print warning about
  // hook options only works with named hook or when using
  // namespace options when adding hook.
  var hookOptions = _.omit(options, 'except', 'namespace', 'only');
  var opts = {};

  if (!_.isEmpty(hookOptions)) {
    if (options.namespace)
      opts[options.namespace] = hookOptions;
    else if (_.isString(hook))
      opts[hook] = hookOptions;
    else {
      console.warn('Hook options only work with named hooks or when '
        + 'using namespace option while adding hooks.');
    }
  }
  ...
  var hookWithOptions = function () {
    ...
    return CurrentOptions.withValue(opts, function () {
      return self.lookupHook(hook).apply(thisArg, arguments);
    });
  };
