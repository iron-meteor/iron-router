...
Router.prototype.addHook = function(type, hook, hookOptions) {
  var self = this;
  ...
  if (hookOptions.only)
    hookOptions.only = toArray(hookOptions.only);
  if (hookOptions.except)
    hookOptions.except = toArray(hookOptions.except);

  var hooks = this._globalHooks[type] = this._globalHooks[type] || [];

  // Add hookOptions to regular options under the hooks name
  var options = {};
  options[hook] = hookOptions;
