...
IronRouter = function (options) {
   ...
   * @api private
   */
  //XXX this should be the current controller instance, not some random context.
  this._current = null;

  /**
   * Dependency to track dependencies on this._current
   * XXX change to _controllerDep
   *
   * @api private
   */
...
IronRouter.prototype = {
  ...
  run: function (path, route, options, cb, onUnhandled) {
    ...
    // this._current may have been reassigned in the onRun function
    // because the user redirected
    if (controller == this._current) {
      cb && cb(context);
      this._deps.changed();
    }
    return this;
