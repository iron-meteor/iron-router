...
//XXX move this into callback it's just easier to reason about.
Router.prototype._redirectToServer = function (url, context) {
  debugger;
  window.location = url;
};
  ...
  var result = controller.dispatch(self._stack, url, function (err) {
    if (err)
      ...
      if (!controller.isHandled() && controller.willBeHandledOnServer()) {
        ...
        // not found message!
        this.render(DEFAULT_NOT_FOUND_TEMPLATE, {data: {url: this.url}});
        return;
      } else {
