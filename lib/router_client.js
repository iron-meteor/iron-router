...
Router.prototype.dispatch = function (url, options) {
  ...
  var result;
  var out = options.next || function () {};

  options.thisArg = controller;
  url = controller.url = controller.path = Url.normalize(url);

  options.next = function (err) {
    // on the client if we get an error we'll just throw it otherwise call out
  ...
  result = controller.dispatch(url, self._stack, options);
