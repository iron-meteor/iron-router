...
SimpleRouteHandler = function (context) {
  ...
  options.template = options.template || context.route.name;
  ...
  //XXX where do filters run? will change on Controllers anyway
  //XXX bug here when used in combination with data macro
...
SimpleRouteHandler.prototype = {
  ...
  // query the data and if it's not found, redirect to
  // notFound template
  //
  // but problem is that it executes the data function outside of
  // the context.result function which is the reactive part, so when 
  // data changes, rendering doesn't happen again.
  // XXX bug here
  // XXX other problem is that the entire route will re-render multiple times
  // for data that is just used in the child template. So this should really
  // be isolated to the child template, and getting data should be delayed
  // if possible until its actually needed by a template. that way, if only
  // a child template uses the data, only the child template is re-rendered
  // not the entire layout and ALL child templates.
  ...
  render: function (templateName, options) {
    ...
    self.context.result(function () {
      //XXX 1. Data not ready on first go around so this function runs
      //reactively
      //XXX 2. outer scope doesn't seem to be available here for some
      //reason. Either that or options are just undefined on the second
      //call
