if (Handlebars) {
  ...
  Handlebars.registerHelper('currentPathIs', function(path) {
    var r = Router.current();
    return r ? Router.current().path === path : false;
  });

  Handlebars.registerHelper('currentRouteIs', function(route) {
    var r = Router.current();
    return r ? Router.current().route.name === route : false;
  });
