...
ClientRouter = RouterUtils.extend(IronRouter, {
  ...
  onRun: function (context, options) {
    ...
    controller = self.getControllerForContext(context, options);
  ...
  start: function () {
    ...
    // make sure all controllers are loaded and createFlushed which means their
    // inheritance hookup is complete before dispatching anything.
    RouteController.afterCreateFlush(function () {
      self.isStarted = true;
