  ...
  Deps.nonreactive(function () {
    ...
    if (activeComp) {
      activeComp.onInvalidate(function () {
        ...
        // subtract from the ready count but do it after any flush
        // cycle. this prevents an infinite invalidation loop in the case
        // where we create a dependency by calling waitlist.ready() and then
        // change the list state in the same computation by calling 
        // waitlist.wait(...) with a function that makes the waitlist not
        // ready. we want to prevent the following within a computation C:
        //  1. Call waitlist.ready() which returns true and creates a dep
        //  2. Call waitlist.wait(function () { return false; }); which invalidates C because the list state has changed
        //  3. C.onInvalidate callback (this callback) called and inner comp is removed from the list
        //  4. C reruns again but this time the state of the list will not change until the next flush cycle so
