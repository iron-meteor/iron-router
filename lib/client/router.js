...
ClientRouter = Utils.extend(IronRouter, {
  ...
  constructor: function (options) {
    ...
    // has the user come cleanly to this URL or is it a hot code reload?
    // this is set to false by the migration code below
    self.hasJustReloaded = true;
  ...
  getData: function () {
    return this.layoutController.getData();
  },
  ...
  run: function (controller, cb) {
    ...
    var maybeRunReactively = function () {
      // we are running this route thanks to a HCR, so keep track.
      controller.isFirstRun = ! self.hasJustReloaded;
      self.hasJustReloaded = false;
