...
// XXX: should be a method on the router I guess
function onclick (e) {
  ...
  // - can be handled client side by the router
  var route = Router.match(href);
  if (! route || route.where !== 'client') 
    return;
...
ClientRouter = Utils.extend(IronRouter, {
  ...
  constructor: function (options) {
    ...
    ClientRouter.__super__.constructor.call(this, options);

    this.isRendered = false;

    this._page = new PageManager;
    this._location = IronLocation;
