...
PageManager.prototype = {
  ...
  run: function (fn, context) {
    ...
    try {
      ...
      var newYieldKeys = _.keys(yields);
      var oldYieldKeys = _.keys(this._yields);
      var clearYieldKeys = _.difference(oldYieldKeys, newYieldKeys);

      if (layout)
        this.layout.set(layout);

      _.each(clearYieldKeys, function (key) {
        self.yieldsToTemplates.set(key, '');
      });

      _.each(newYieldKeys, function (key) {
        self.setTemplate(yields[key], key);
      });
