...
SubscriptionManager.prototype._closeRoute = function(route) {
  //delete from the main
  delete this._routes[route.hash];
  var index = this._routeCache[route.name].indexOf(route);
  this._routeCache[route.name].splice(index, 1);
};

SubscriptionManager.prototype._closeSub = function(sub) {
  if(this._subscriptions[sub.hash]) {
    //need to find out whether is this used by other routes
    var noOneIsUsing = true;
    for(var lc=0; lc<this._routes.length; lc++) {
      var route = this._routes[lc];
      if(route.existsSubscription(sub.hash)) {
        noOneIsUsing = false;
        break;
      }
    }
    ...
    if(noOneIsUsing) {
      ...
      var usedInCurrentRoute = this._currentRoute && this._currentRoute.existsSubscription(sub.hash);
