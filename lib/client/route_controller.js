RouteController = Utils.extend(RouteController, {
   ...
   * data context. This is what controller functions will read from. Templates
   * will get their data context from the global router data context which will
   * get set in the _run function.
   */
  data: function () {
    var self = this;
    var value;
  ...
  _run: function () {
    ...
    self._computation = Deps.autorun(withNoStopsAllowed(function () {
      ...
      // data
      // we run the set data step at the end to give any new templates
      // a chance to render first. If we do this step first, then the old
      // templates might re-render because of a data change, before the new
      // templates have a chance to render, because the data computation reruns
      // first. Note that controller functions (e.g. hooks) still get their data
      // from the local controller data function so they can call this.data()
      // any time.
      Deps.autorun(withNoStopsAllowed(function () {
        if (self.ready()) {
          ...
          // set the global router data which is
          // what templates and the layout will get
          // their data contexts from.
