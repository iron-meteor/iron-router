  ...
  _run: function () {
    ...
    self._computation = Deps.autorun(withNoStopsAllowed(function () {
      ...
      function clearUnusedRegions () {
        var self = this;
        // XXX just a hack for now. we need all of the rendering to complete
        // before we can see which regions were rendered (so we need a
        // Deps.flush). But we can't flush inside a Deps.autorun. So we'll wait
        // until this tick is complete and then clear unused regions.
        setTimeout(function () {
          var regions = _.keys(self.router._layout.endRendering());
      ...
      self.runHooks('onBeforeAction', [], function (paused) {
        ...
        if (paused) {
          clearUnusedRegions.call(this);
        } else {
          action.call(self);

          if (!self.isStopped) {
            self.runHooks('onAfterAction', [clearUnusedRegions]);
          }
        }
      });
    }));
