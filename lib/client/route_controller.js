_.extend(RouteController.prototype, {
  ...
  render: function (template, options) {
    ...
    self.router.setTemplate(template, to);
  },
  ...
  run: function () {
    ...
    var onDataReady = function () {
      // just execute the data function once instead of once per render
      ...
      if (self.yieldTemplates) {
        _.each(self.yieldTemplates, function (options, template) {
    ...
    if (this.layoutTemplate) 
      this.router.setLayout(self.layoutTemplate);

    this.wait(this.waitOn, onDataReady, onWaiting);
  },
  ...
  wait: function (handles, onReady, onWait) {
    ...
    if (_.isFunction(handles)) {
      ...
      if (!handles) {
        throw new Error(
          'Oh goodness, it looks like your waitOn function is not returning anything!'
        );
      }
    }
