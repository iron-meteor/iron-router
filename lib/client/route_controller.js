...
RouteController = Utils.extend(RouteController, {
  ...
  _stopController: function (cb) {
    ...
    self.router._layout.endRendering({flush: false});
  ...
  _run: function () {
    ...
    var finishCurrentRenderingTransaction = null;
    ...
    self._computation = Deps.autorun(withNoStopsAllowed(function () {
      ...
      Deps.autorun(function (c) {
        if (finishCurrentRenderingTransaction)
          finishCurrentRenderingTransaction();
        ...
        self.router._layout.beginRendering();

        // this could get called either in the Deps.afterFlush callback, or from
        // above if this computation reruns before the after flush is called. So
        // we want the function to be idempotent (only run once). On each
        // computation run, we want to clear any regions that were not rendered.
        finishCurrentRenderingTransaction = _.once(function () {
          var regions = self.router._layout.endRendering({flush: false});
          self.router.clearUnusedRegions(regions);
        });

        Deps.afterFlush(finishCurrentRenderingTransaction);
