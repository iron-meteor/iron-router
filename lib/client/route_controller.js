...
RouteController = function (context, options) {
  ...
  this.loadingTemplate = options.loadingTemplate 
    || routeOptions.loadingTemplate 
    || routerOptions.loadingTemplate 
    || this.loadingTemplate;

  this.notFoundTemplate = options.notFoundTemplate 
    || routeOptions.notFoundTemplate
    || routerOptions.notFoundTemplate
  ...
  this.onBeforeRun = options.onBeforeRun
    || routeOptions.onBeforeRun
    || routerOptions.onBeforeRun
    || this.onBeforeRun;

  this.onAfterRun = options.onAfterRun
    || routeOptions.onAfterRun
    || routerOptions.onAfterRun
    || this.onAfterRun;

  this.onBeforeRerun = options.onBeforeRerun
    || routeOptions.onBeforeRerun
    || routerOptions.onBeforeRerun
    || this.onBeforeRerun;

  this.onAfterRerun = options.onAfterRerun
    || routeOptions.onAfterRerun
    || routerOptions.onAfterRerun
    || this.onAfterRerun;

  this.template = options.template
    || routeOptions.template
    || routerOptions.template
    || this.template
...
_.extend(RouteController, {
  ...
  create: function (definition) {
    return RouterUtils.create(definition);
  },

  _isCreateFlushed: false,

  _isCreateFlushScheduled: false,

  _onCreateFlushQueue: [],

  _afterCreateFlushQueue: [],

  _createFlush: function () {
    var flushQueue = RouteController._onCreateFlushQueue;

    for (var i = 0; i < flushQueue.length; i++) {
      //XXX right now we're not guaranteeing that another function
      //can't call flush again which would put us in an infinite loop. but this
      //is okay for now because these are private methods.
      flushQueue[i]();
    }

    RouteController._isCreateFlushed = true;

    var afterQueue = RouteController._afterCreateFlushQueue;
  ...
  _scheduleCreateFlush: function () {
    if (!RouteController._isCreateFlushed &&
        !RouteController._isCreateFlushScheduled) {
      Meteor.startup(function () {
        setTimeout(function () {
          RouteController._createFlush();
        });
      });
      RouteController._isCreateFlushScheduled = true;
    }
  },

  onCreateFlush: function (cb) {
    if (RouteController._isCreateFlushed)
      cb();
    else {
      RouteController._onCreateFlushQueue.push(cb);
      RouteController._scheduleCreateFlush();
    }
  },

  afterCreateFlush: function (cb) {
    // already flushed or queue is empty
    if (RouteController._isCreateFlushed ||
        !RouteController._onCreateFlushQueue.length)
      cb();
    // otherwise push onto the queue to be executed later
    else
      RouteController._afterCreateFlushQueue.push(cb);
  }
});
