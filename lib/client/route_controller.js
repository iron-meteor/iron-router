RouteController = Utils.extend(RouteController, {
  constructor: function () {
    ...
    // wraps router.data except it also sets the controller's private _dataValue
    // property if a data value is provided. This is because we call
    // _getControllerData() when setting the router's data in the run method. We
    // need some way of getting the initial data value before wiring up the data
    // method to router.data. But if the user changes data by calling
    // this.data({title: 'some title'}) for example, and *later* the controller
    // is marked as ready and the global data context gets set, we want to use
    // the new data value.
    this.data = function (value) {
      if (value)
        this._dataValue = value;
      return this.router.data.apply(this, arguments);
    };
  ...
  _stopController: function (cb) {
    ...
    var onStop = function () {
      ...
      // set the isStopped state after we
      // run the onStop hooks.
      self.isStopped = true;
      cb && cb.call(self);
    };
  ...
  /**
   * Get the data from the controller itself, not the router global
   * data context.
   */
  _getControllerData: function () {
    var self = this;
    var value;
    ...
    self._computation = Deps.autorun(withNoStopsAllowed(function () {
      ...
      Deps.autorun(withNoStopsAllowed(function () {
        if (self.ready()) {
          ...
          // get the local controller data
          var data = self._getControllerData();

          // now take that and set the global router
          // data context which is what templates and other
          // controller functions will use. i.e. when you
          // call this.data() or this.data({...}) you're calling
          // that method on the router which in turn is calling it
          // on the ui manager.
          self.router.data(data);

          // let people plug into the process
