...
RouteController = Utils.extend(IronRouteController, {
    ...
    if (arguments.length == 0) {
      ...
      if (this.yieldTemplates) {
        _.each(this.yieldTemplates, function (opts, tmpl) {
          self.router.setTemplate(tmpl, opts);
        });
      }
    } else {
      options = options || {};
  ...
  run: function () {
     ...
     */
    waitOn = _.flatten(_.map(waitOn, function (fnOrHandle) {
      return _.isFunction(fnOrHandle) ? fnOrHandle.call(self) : fnOrHanlde;
    }));
