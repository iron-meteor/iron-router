...
/**
 * A data structure for tracking whether an entire list of ready handles is
 * marked as ready without invalidating the outer computation for each time a
 * ready state changes. It only invalidates the dependency when the entire state
 * changes from ready to not ready and visa versa.
 */
...
WaitList.prototype = {
  ...
  push: function (o) {
    ...
    var comp = Deps.autorun(function (c) {
      // what if o.ready() goes back to false at a later time?
      // do we increment the counter?
      var ret = o.ready();
      if (ret && self._waitCount > 0)
        self._waitCount--;
      else if (!ret)
        self._waitCount++;

      var isReady = self._waitCount == 0;
...
var autoClearUnusedYieldsHook = function () {
  this.router.clearUnusedYields(this._renderedYields);
};
...
RouteController = Utils.extend(IronRouteController, {
  ...
  render: function (template, options) {
    ...
    if (arguments.length == 0) {
      this.router.setTemplate(this.template);
      // XXX use a constant property here
      this._renderedYields.push('__main__');

      if (this.yieldTemplates) {
        _.each(this.yieldTemplates, function (opts, tmpl) {
          self.router.setTemplate(tmpl, opts.to);
          self._renderedYields.push(opts.to);
        });
      }
    } else {
      options = options || {};
