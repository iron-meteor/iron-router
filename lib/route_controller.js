var rewriteLegacyHooks = function (controller) {
  ...
  _.each(legacyToNew, function (newHook, oldHook) {
    ...
    if (_.has(controller.options, oldHook)) {
      hasOld = true;
      controller.options[newHook] = controller.options[oldHook];
    }

    // search on the object and the proto chain
    if (typeof controller[oldHook] !== 'undefined') {
      hasOld = true;
      controller[newHook] = controller[oldHook];
    }
...
RouteController.prototype = {
  ...
  runHooks: function (hookName, more, cb) {
    ...
    // 1. Router global hooks
    // 2. Route option hooks
    // 3. RouteController option hooks
    // 4. RouteController proto hooks (including inherited super to child)
    // 5. RouteController object hooks
    // 6. more
