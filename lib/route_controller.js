...
RouteController.prototype = {
  ...
  runHooks: function (hookName, more, cb) {
    ...
    var hookNames = Utils.toArray(hookName);
    var isPaused = false;
    _.each(hookNames, function (name) {
      // last isPaused wins
      isPaused = self._runHook(name, more);
    });

    cb && cb.call(self, isPaused);
  },

  _runHook: function (hookName, more) {
    var self = this;
    ...
    var objectHooks = toArray(this[hookName]);
