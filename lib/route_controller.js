IronRouteController = function (router, options) {
  ...
  this.hooks = {};
  this.hooks.before = RouterUtils.toArray(options.before);
  this.hooks.after = RouterUtils.toArray(options.after);
};
...
_.extend(IronRouteController, {
  ...
  before: function (fn) {
    ...
    this.hooks.before = this.hooks.before.concat(RouterUtils.toArray(fn));
  },

  after: function (fn) {
    this.hooks = this.hooks || {};
    this.hooks.after = this.hooks.after || [];
    this.hooks.after = this.hooks.after.concat(RouterUtils.toArray(fn));
  }
});
