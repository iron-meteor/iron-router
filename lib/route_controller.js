 ...
 *   this.router.options
 */
RouteController.prototype.lookupOption = function (key, ns) {

  if (typeof ns !== 'undefined') {

    // this.options
    if (this.options && this.options[ns] && _.has(this.options[ns], key))
      return this.options[ns][key];

    // "this" object or its proto chain
    if (this[ns] && typeof this[ns][key] !== 'undefined')
      return this[ns][key];

    // see if we have the CurrentOptions dynamic variable set.
    var opts = CurrentOptions.get();
    if (opts && opts[ns] && _.has(opts[ns], key))
      return opts[ns][key];

    // this.route.options
    if (this.route && this.route.options && this.router.options[ns] && _.has(this.route.options[ns], key))
      return this.route.options[ns][key];

    // this.router.options
    if (this.router && this.router.options && this.router.options[ns] && _.has(this.router.options[ns], key))
      return this.router.options[ns][key];
  } else {
