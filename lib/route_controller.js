...
RouteController.prototype.lookupOption = function (key) {
  ...
  var resolve = Iron.utils.resolve;
  var resolvedPath;
  var options = {
    returnObject: true
  };
  ...
  if (this.route && this.route.options) {
    resolvedPath = resolve(key, this.route.options, options);
    if (resolvedPath && resolvedPath.keyExists)
      return resolvedPath.value;
  }

  // this.options
  resolvedPath = resolve(key, this.options, options);
  if (resolvedPath && resolvedPath.keyExists)
    return resolvedPath.value;

  // "this" object or its proto chain
  resolvedPath = resolve(key, this, options);
  if (resolvedPath && resolvedPath.keyExists)
    return resolvedPath.value;
  ...
  if (opts) {
    resolvedPath = resolve(key, opts, options);
    if (resolvedPath && resolvedPath.keyExists)
      return resolvedPath.value;
  }

  // this.router.options
  if (this.router && this.router.options) {
    resolvedPath = resolve(key, this.router.options, options);
    if (resolvedPath && resolvedPath.keyExists)
      return resolvedPath.value;
  }
